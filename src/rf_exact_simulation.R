...
#Simulate matchup with (exact) restoring force
exact_rf_simulate_matchup <- function(team_a, team_b, c) {
  #Running tally of plus/minus
  overall_plus_minus = 0
  ...
  while (a_T < game_duration || b_T < game_duration) {
    ...
    overall_coef = team_lineup_coef[[team_a]][a_prev_state] - team_lineup_coef[[team_b]][b_prev_state]
    overall_plus_minus = (overall_coef - ((overall_coef - (c * overall_plus_minus)) * exp(-c * time_interval))) / c
    
  } #end while loop
  return(overall_plus_minus)
}

exact_rf_simulate_matchup("Chi", "Was", 0.001)

#Function for simulating best of 7 series with restoring force
exact_rf_simulate_series <- function(team_a, team_b, c) {
  a_wins = 0
  b_wins = 0
  while (a_wins < 4 && b_wins < 4) {
    differential = exact_rf_simulate_matchup(team_a, team_b, c)
    if (differential > 0) {
      a_wins = a_wins + 1
...
for (i in 1:100) {
  if (exact_rf_simulate_series("Chi", "Was", 0.001) == "Was") {
